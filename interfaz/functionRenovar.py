...
import functionAgendar as agendar

def renovar(TIEMPO_EXTENSION):
	KEYS_SIZE		= 16
	...
	try:
	    # TIEMPO_EXTENSION = Cantidad de dIas que se extiende DESDE FECHA ACTUAL.

	    os.system("nfc-mfclassic r a userCard.mfd > logs.txt")
	    file = open("userCard.mfd","rb")
	    crudo = ""
	    file.seek(ENTIRE_BLOCK_SIZE, 0)
	    for i in range(1, 1008): ##1024-16, es lo mismo en realidad.
		crudo += file.read(48)
	    ...
	    fecha_final = dt.today() + timedelta(days=int(TIEMPO_EXTENSION))  
	    valid_thru_ok = 1


	    # PREPARO STRING FINAL
	    data_completa = "[" + crudo.split('[')[1].split(']')[0] + "][" + fecha_final.strftime("%d/%m/%Y") + "]"
		...
		print ("INICIANDO PROCESO DE RENOVACION.")
	    else:
	    ...
	    # IMPRESION DE DATOS.
	    print ("Hoy es " + dt.today().strftime("%d/%m/%Y"))
	    print ("La fecha de caducidad de la tarjeta es el " + validthru)
	    print ("La nueva fecha sera asignada para el " + fecha_final.strftime("%d/%m/%Y"))



	    # GENERO UNA AGENDACION CON LOS DATOS NUEVOS.
	    print(crudo.split('[')[1].split(']')[0])
	    agendado_ok = agendar.agendarUsuario(crudo.split('[')[1].split(']')[0], fecha_final.strftime("%d/%m/%Y"))
	    if(agendado_ok == 0):
		print ("RenovaciOn completada exitosamente.")
	    else:
		print ("RenovaciOn FALLIDA.")
