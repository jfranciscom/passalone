...
def agendarUsuario(rawData, fechaVenc):
	...
	try:
	    ...
	    datosUser = "[" + rawData + "][" + str(fechaVenc) + "]"
	    
	    ## COMPRUEBO QUE LA FECHA DE RENOV. NO ESTE MAL.
		# Puede estar incorrecta o caducada.

	    venc = dt.strptime(fechaVenc, "%d/%m/%Y") # Parsea bien o error.
	    if (venc < dt.today()):
		return 1
